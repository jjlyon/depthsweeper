...
import { addItem, countItem, hasItem } from './effects';
...
export function buyItem(run:RunState,itemId:string): boolean { const item=ITEM_BY_ID[itemId]; if(!item?.price || run.player.gold<item.price) return false; const owned=countItem(run,itemId); if(owned>0 && item.maxStacks !== undefined && owned>=item.maxStacks) return false; run.player.gold-=item.price; addItem(run,itemId); return true; }
